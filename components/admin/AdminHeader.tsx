...
export const AdminHeader: React.FC = () => {
  ...
  return (
    <header className="flex justify-between items-center">
        <h1 className="text-xl font-semibold text-brand-text-primary-light dark:text-white">Admin Panel</h1>
        <div className="flex items-center space-x-4">
            ...
            <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center space-x-2">
                    <div className="w-10 h-10 rounded-full bg-brand-accent flex items-center justify-center font-bold text-brand-primary">
                        {user?.name.charAt(0).toUpperCase()}
                    </div>
                </button>
                {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-brand-secondary-light dark:bg-brand-secondary rounded-lg shadow-lg border border-brand-border-light dark:border-brand-border py-2 z-50">
